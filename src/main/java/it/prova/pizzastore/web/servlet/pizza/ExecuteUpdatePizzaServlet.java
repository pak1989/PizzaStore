...
@WebServlet("/pizza/ExecuteUpdatePizzaServlet")
public class ExecuteUpdatePizzaServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		PizzaDTO pizzaDTO = new PizzaDTO(id, descrizione, codice);
		...
		Pizza pizzaDaAggiornare = pizzaService.caricaSingoloPizza(id);

		try {
			PropertyUtils.copyProperties(pizzaDaAggiornare, pizzaDTO);
			System.out.println(pizzaDaAggiornare.toString());
		} catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
			// TODO Auto-generated catch block
			System.out.println("COPIA PROPIETA' FALLITA");
		}

		pizzaService.aggiorna(pizzaDaAggiornare);

		request.setAttribute("singolaPizza", pizzaService.caricaSingoloPizza(id));
